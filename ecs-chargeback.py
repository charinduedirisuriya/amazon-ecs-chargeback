...
def get(table, region, cluster, service, db):
    """
    Scan the MySQL table to get all tasks in a service.
    Input - region, ECS ClusterARN and ECS ServiceName
    """
    data = {'Items':[]}
    sql  = "SELECT * FROM task_definitions WHERE clusterArn = '"+cluster+"' AND region = '"+region+"' AND groupName = '"+service+"' AND `group` = 'service'"
    mycursor = db.cursor()
...
if __name__ == "__main__":
    ...
    print(cli_args.month)
    print(cli_args.days)
    print(cli_args.hours)
    ...
    tasks = get(table, region, cluster, service, database)
